...
class PurchaseService {
  ...
  getAllMedicinesByFilter = async (medicine) => {
    ...
    const queryRef = await query(
      medicineCollectionRef,
      where("brandName", "==", medicine?.brandName),
      where("dose", "==", medicine?.dose),
      where("form", "==", medicine?.form)
    );
  ...
  medicineById = async (vendorId, medicineId) => {
    ...
    if (medicineId && vendorId && medicineId !== "" && vendorId !== "") {
      ...
      queryRef = query(quotationCollectionRef, orderBy("ptr", "asc"), limit(5));
    }
    const querySnapshot = await getDocs(queryRef);
    const filteredData = querySnapshot.docs.map((doc) => ({
      ...doc.data(),
